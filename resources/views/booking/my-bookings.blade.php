...
@section('content')
            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">Kelola semua pesanan rental kendaraan Anda</p>
                    </div>

                    <!-- Button Kembali -->
                    <a href="{{ route('home') }}"
                        class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all duration-200 shadow-md hover:shadow-lg">
            ...
            <div class="space-y-3 sm:space-y-4">
                @forelse($bookings as $booking)
                    <div class="booking-card bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden hover:shadow-lg transition"
                        ...
                        <div class="p-4 sm:p-6">
                            <!-- Header -->
                            <div
                                class="flex flex-col sm:flex-row justify-between items-start mb-4 pb-3 sm:pb-4 border-b gap-3 sm:gap-4">
                                <div class="w-full sm:w-auto">
                                    <div class="flex items-center gap-2 sm:gap-3 mb-2 flex-wrap">
                                        <h3 class="text-base sm:text-lg font-bold text-gray-900">
                                            {{ $booking->nomor_booking }}</h3>
                                        @if ($booking->status == 'pending')
                                            <span
                                                class="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-yellow-100 text-yellow-800 text-xs font-semibold rounded-full whitespace-nowrap">
                                                PENDING
                                            </span>
                                        @elseif($booking->status == 'dikonfirmasi')
                                            <span
                                                class="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-blue-100 text-blue-800 text-xs font-semibold rounded-full whitespace-nowrap">
                                                DIKONFIRMASI
                                            </span>
                                        @elseif($booking->status == 'dalam_perjalanan')
                                            <span
                                                class="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-green-100 text-green-800 text-xs font-semibold rounded-full whitespace-nowrap">
                                                DALAM PERJALANAN
                                            </span>
                                        @elseif($booking->status == 'selesai')
                                            <span
                                                class="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-gray-100 text-gray-800 text-xs font-semibold rounded-full whitespace-nowrap">
                                                SELESAI
                                            </span>
                                        @else
                                            <span
                                                class="px-2.5 sm:px-3 py-0.5 sm:py-1 bg-red-100 text-red-800 text-xs font-semibold rounded-full whitespace-nowrap">
                                                DIBATALKAN
                                            </span>
                                        @endif
                                    </div>
                            ...
                            <div class="grid md:grid-cols-2 gap-4 sm:gap-6">
                                <!-- Kendaraan Info -->
                                <div>
                                    <h4 class="font-semibold text-gray-900 mb-3 text-sm sm:text-base">Informasi Kendaraan
                                    </h4>
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <div class="flex gap-2 sm:gap-3 text-xs text-gray-600 mt-1">
                                                <span class="whitespace-nowrap"><i
                                                        class="fas fa-users mr-1"></i>{{ $booking->kendaraan->kapasitas_penumpang }}</span>
                                                <span class="whitespace-nowrap"><i
                                                        class="fas fa-cog mr-1"></i>{{ ucfirst($booking->kendaraan->transmisi) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2 text-xs sm:text-sm">
                                        <div class="flex justify-between gap-2">
                                            ...
                                            <span
                                                class="font-medium text-right">{{ \Carbon\Carbon::parse($booking->tanggal_mulai)->isoFormat('D MMM Y') }}</span>
                                        </div>
                                        <div class="flex justify-between gap-2">
                                            <span class="text-gray-600 whitespace-nowrap">Tanggal Selesai:</span>
                                            <span
                                                class="font-medium text-right">{{ \Carbon\Carbon::parse($booking->tanggal_selesai)->isoFormat('D MMM Y') }}</span>
                                        </div>
                                        <div class="flex justify-between gap-2">
                            ...
                            <!-- Actions -->
                            <div
                                class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 mt-4 sm:mt-6 pt-3 sm:pt-4 border-t">
                                ...
                                @if (in_array($booking->status, ['pending', 'dikonfirmasi']))
                                    <button
                                        onclick="cancelBooking('{{ $booking->id }}', '{{ $booking->nomor_booking }}')"
                                ...
                                @if ($booking->status == 'selesai')
                                    <button
                                        class="flex-1 sm:flex-none text-center px-4 sm:px-6 py-2 bg-indigo-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-indigo-700 transition whitespace-nowrap">
                ...
                @empty
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-8 sm:p-12 text-center">
                        ...
                        <p class="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Anda belum memiliki riwayat booking
                            kendaraan</p>
                        <a href="{{ route('home') }}"
    ...
    <!-- Cancel Modal -->
    <div id="cancelModal"
        class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-3 sm:p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
            ...
            <p class="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4">Apakah Anda yakin ingin membatalkan booking ini?
